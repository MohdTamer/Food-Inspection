...
def show_distributions():
    ...
    with tab2:
        ...
        fig = px.bar(x=risk.index.astype(str), y=risk.values, color=risk.values, title="Count by Risk Level")
        st.plotly_chart(fig, use_container_width=True)
        
    with tab3:
        it = payload["inspection_type_distribution"]
        st.plotly_chart(px.treemap(names=it.index, parents=["All"]*len(it), values=it.values), use_container_width=True)
